...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        # Update the LEDs so the thermostat shows the correct heat status.
        self.updateLights()
    ...
    def on_exit_heat(self):
        # Turn the red light off when the thermostat leaves heat mode.
        redLight.off()
    ...
    def on_enter_cool(self):
        # Update the LEDs so the thermostat shows the correct cool status.
        self.updateLights()
    ...
    def on_exit_cool(self):
        # Turn the blue light off when the thermostat leaves cool mode.
        blueLight.off()
    ...
    def on_enter_off(self):
        # Turn off both LEDs because off mode should not show heat or cool.
        redLight.off()
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        # Move to the next thermostat mode.
        self.cycle()

        # Show the new mode on the LCD right away.
        self.showButtonUpdate(f"State: {self.current_state.id}")
    ...
    def processTempIncButton(self):
        ...
        # Raise the set temperature by one degree.
        self.setPoint = self.setPoint + 1

        # Refresh the LEDs in case the new setpoint changes the light behavior.
        self.updateLights()

        # Update the LCD with the new set temperature.
        self.showButtonUpdate(f"Set Temp: {self.setPoint} F")
    ...
    def processTempDecButton(self):
        ...
        # Lower the set temperature by one degree.
        self.setPoint = self.setPoint - 1
    ...
    def updateLights(self):
        ...
        # Heat mode uses the red LED to show heating behavior.
        if(self.current_state.id == "heat"):
            # Blink the red LED when the room is colder than the setpoint.
            if(temp < self.setPoint):
                redLight.pulse()

            # Keep the red LED solid when heat is selected but not actively heating.
            else:
                redLight.on()

        # Cool mode uses the blue LED to show cooling behavior.
        elif(self.current_state.id == "cool"):
            # Blink the blue LED when the room is hotter than the setpoint.
            if(temp > self.setPoint):
                blueLight.pulse()

            # Keep the blue LED solid when cool is selected but not actively cooling.
            else:
                blueLight.on()
    ...
    def setupSerialOutput(self):
        # Create a comma-delimited status message for the serial server.
    ...
    ##
    ## Temporary display override used after a button press.
    ## This lets the LCD show the user's change right away before
    ## going back to the normal temperature/state display.
    ##
    overrideDisplayText = None
    overrideDisplayCounter = 0

    ##
    ## showButtonUpdate - Helper method used after a button press.
    ## This updates the LCD right away and keeps the message visible
    ## for a few cycles so it is easier to see.
    ##
    def showButtonUpdate(self, line_2):
        current_time = datetime.now()
        lcd_line_1 = current_time.strftime("%m/%d %H:%M") + "\n"

        self.overrideDisplayText = line_2
        self.overrideDisplayCounter = 3

        screen.updateScreen(lcd_line_1 + line_2)
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # Format the first LCD row with the current date and time.
            lcd_line_1 = current_time.strftime("%m/%d %H:%M") + "\n"

            ## Setup Display Line 2
            if(self.overrideDisplayCounter > 0):
                # Keep the button update on the screen for a few seconds.
                lcd_line_2 = self.overrideDisplayText
                self.overrideDisplayCounter = self.overrideDisplayCounter - 1

            elif(altCounter < 6):
                # Show the current temperature on the second LCD row.
                lcd_line_2 = f"Temp: {floor(self.getFahrenheit())} F"

                altCounter = altCounter + 1
            else:
                # Show the current thermostat state and setpoint on the second LCD row.
            ...
            if((counter % 30) == 0):
                # Send the thermostat status string over UART as bytes.
...
# Cycle the thermostat state asynchronously when pressed
greenButton.when_pressed = lambda: Thread(target=tsm.processTempStateButton).start()
...
# Increase the thermostat setpoint asynchronously when pressed
redButton.when_pressed = lambda: Thread(target=tsm.processTempIncButton).start()

##
## Configure our Blue button to use GPIO 19 and to execute
## the function to decrease the setpoint by a degree.
##
blueButton = Button(19, bounce_time=0.3)
# Decrease the thermostat setpoint asynchronously when pressed
blueButton.when_pressed = lambda: Thread(target=tsm.processTempDecButton).start()
